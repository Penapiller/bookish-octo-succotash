  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-3">
        {entries.map((entry) => (
          ...
          >
            <summary className="flex cursor-pointer flex-wrap items-center gap-2">
              <span className="font-medium uppercase text-xs tracking-wide text-zinc-500">
                {entry.action_type}
              </span>
        ...
        {entries.length === 0 ? (
          <p className="py-6 text-center text-sm text-zinc-500">No admin actions logged yet.</p>
        ) : null}
      </div>
    </div>
  );
